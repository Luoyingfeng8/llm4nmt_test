...
class LlamaCrossAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        cos_q, sin_q = self.rotary_emb(value_states, position_ids)
        query_states = apply_rotary_pos_emb_single(query_states, cos_q, sin_q)

        # get key's position embedding
        cos_k, sin_k = self.rotary_emb(value_states, encoder_position_ids)
...
class LlamaConAttLWDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ## concat hidden: training and first inference step
        is_first_inference_step = cache_position.size(0) == attention_mask.size(-1)

        if self.training or is_first_inference_step:
...
class LlamaCrossAttLWDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class LlamaParallelCrossAttLWDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        self_hidden_states, self_attn_weights, present_key_value = self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
        ...
        # cross Attention
        cross_hidden_states, cross_attn_weights, cross_present_key_value = self.cross_attn(
            hidden_states=hidden_states,
            position_ids=position_ids,
            output_attentions=output_attentions,
            use_cache=use_cache,
        ...
        coefficient = torch.sigmoid(self.alpha)

        # fuse
        hidden_states =  coefficient * self_hidden_states + (1-coefficient) * cross_hidden_states
...
## decoder-only as Encoder with addtional cross attention
class LlamaModelCrossEncoder(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList(
            [LlamaParallelCrossAttLWDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        ### encoder_attention_mask is equal to attention_mask when they has the same vocabulary
...
class Connector(nn.Module):
    def __init__(self, config):
        ...
        if self.encoder_method == "stack":
            # stack_encoder_config copy from encoder temp
            tiny_encoder_config = copy.deepcopy(config.decoder)
            # tiny_encoder_config["hidden_size"] = 2048
            # tiny_encoder_config["intermediate_size"] = 5504
            ...
            # self.project2 = ProjectDown(2048, 1024)
        elif self.encoder_method == "cross_atten":
            cross_encoder_config = PretrainedConfig.from_dict(config.decoder)  
            self.post_encoder = LlamaModelCrossEncoder(cross_encoder_config)
    ...
    def forward(
        ...
        if self.encoder_method == "stack":
            ...
            post_encoder_outputs = self.post_encoder(
                ...
                output_hidden_states=True,
                return_dict=return_dict,
                encoder_all_hidden_states=[hidden],
...
## PrefixLM: Encoder(embedding) + LlamaConAttT2BPrefixDecoder
class LlamaModelEmbeddingEncoder(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
...
# ConcatAttentionToptoBottomDecoder
class LlamaConAttT2BDecoder(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        # training use, inference will provide
        if cache_position is None:
            if isinstance(past_key_values, StaticCache):
                raise ValueError("cache_position is a required argument when using StaticCache.")
            # cache_position = torch.arange(
        ...
        if position_ids is None:
            ...
            ## reset position
            # position_ids = torch.cat([encoder_attention_mask.long().cumsum(-1)-1, decoder_attention_mask.long().cumsum(-1)-1], dim=1)
        ...
        # only cat when in training or in first inference step
        if self.training or is_first_inference_step:
            # cat encoder top layer's hidden state
            # start_idx = self.config.num_hidden_layers + 1
            start_idx = 1  # last hidden stat
            hidden_states = torch.cat([encoder_all_hidden_states[-start_idx], hidden_states], dim=1)            
        ...
        if self.training or is_first_inference_step:
            hidden_states = hidden_states[:, encoder_attention_mask.size(1):, :]
    ...
    def _update_causal_mask(self, attention_mask, input_tensor, cache_position, past_seen_tokens):
        ...
        causal_mask *= torch.arange(target_length, device=device) > cache_position.reshape(-1, 1) # get a triu 
        causal_mask = causal_mask[None, None, :, :].expand(input_tensor.shape[0], 1, -1, -1)
        if attention_mask is not None:
            causal_mask = causal_mask.clone()  # copy to contiguous memory for in-place edit
            if attention_mask.dim() == 2:
...
class LlamaConAttT2BPrefixDecoder(LlamaConAttT2BDecoder):
    ...
    def forward(
        ...
        causal_mask = self._update_causal_mask(attention_mask, inputs_embeds, cache_position, past_seen_tokens, encoder_attention_mask)
        # torch.set_printoptions(threshold=10_000, precision=3, edgeitems=3, linewidth=200)
    ...
    def _update_causal_mask(self, attention_mask, input_tensor, cache_position, past_seen_tokens, encoder_attention_mask=None):
        if self.config._attn_implementation == "flash_attention_2":
            if attention_mask is not None and 0.0 in attention_mask:
                return attention_mask
            return None
        ...
        # prefix attention
        prefix_length = encoder_attention_mask.shape[-1]
        causal_mask[:, :prefix_length] = 0
...
# ConcatAttentionLayerWiseDecoder
class LlamaConAttLWDecoder(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList(
            [LlamaConAttLWDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        ## old code 
        self.share_bottom_k = getattr(config, "share_bottom_k", 0) 
        self.share_top_m = getattr(config, "share_top_m", 0) 
        self.share_middle = getattr(config, "share_middle", 0)
        self.encoder_layer_num = getattr(config, "num_encoder_layers", 32)

        ## new code 
        self.share_encoder_layers_id = getattr(config, "share_encoder_layers_id", None)
    ...
    def forward(
        ...
        # print(causal_mask.size())
        # print(encoder_all_hidden_states[0].size())
                
        # share_bottom_k +  share_top_m
        if self.share_encoder_layers_id:
            fuse_encoder_hidden_states = [encoder_all_hidden_states[i] for i in self.share_encoder_layers_id]
            
            ## for Top-To-All Connection
            # fuse_encoder_hidden_states = [encoder_all_hidden_states[-1] for i in self.share_encoder_layers_id]
        elif self.config.model_method == "TinyConAttLW":
            fuse_encoder_hidden_states = [encoder_all_hidden_states[-1] for _ in range(len(self.layers))]
        else:
            if self.share_middle == 0:
                sample_middle_layers = tuple()
            # share_bottom_k + middle + share_top_m
            else:
                interval = (self.encoder_layer_num - self.share_bottom_k - self.share_top_m) // self.share_middle
                sample_middle_layers_id = [self.share_bottom_k+i*interval for i in range(self.share_middle)]
                sample_middle_layers = tuple([encoder_all_hidden_states[i] for i in sample_middle_layers_id])
                fuse_encoder_hidden_states = encoder_all_hidden_states[:self.share_bottom_k] + sample_middle_layers + \
                    encoder_all_hidden_states[-(self.share_top_m+1):-1]
...
class LlamaCrossAttLWDecoder(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList(
            [LlamaCrossAttLWDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        ## old code 
        self.share_bottom_k = getattr(config, "share_bottom_k", 0) 
        self.share_top_m = getattr(config, "share_top_m", 0) 
    ...
    def forward(
        ...
        if self.share_encoder_layers_id:
            ...
            ## for Top-To-All Connection
            # fuse_encoder_hidden_states = [encoder_all_hidden_states[-1] for i in self.share_encoder_layers_id]
        # only use the last_encoder_hidden to fuse
        elif self.config.model_method == "TinyCrossAttLW":
...
class TinyLlamaConAttLWDecoder(LlamaConAttLWDecoder):
    def __init__(self, config):
        super().__init__(config)

class TinyLlamaConAttT2BDecoder(LlamaConAttT2BDecoder):
    def __init__(self, config):
        super().__init__(config)

class TinyLlamaConAttT2BPrefixDecoder(LlamaConAttT2BPrefixDecoder):
    def __init__(self, config):
        super().__init__(config)

class TinyLlamaCrossAttLWDecoder(LlamaCrossAttLWDecoder):
    def __init__(self, config):
        super().__init__(config)
        
        
### all Llama Encoder-Decoder
class LlamaForEncDec(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        elif encoder_method == "prefix":
            self.encoder = LlamaModelEmbeddingEncoder(config)
        self.lm_head = None

        ## decoder
        ### for share decoder
        decoder_config = PretrainedConfig.from_dict(config.decoder) if getattr(config, "decoder", None) is not None else config
        if decoder_config.model_method == "ConAttLW":
            self.decoder = LlamaConAttLWDecoder(decoder_config)
        elif decoder_config.model_method == "ConAttT2B":
            self.decoder = LlamaConAttT2BDecoder(decoder_config)
        elif decoder_config.model_method == "CrossAttLW":
            self.decoder = LlamaCrossAttLWDecoder(decoder_config)
        ### for light decoder
        elif decoder_config.model_method == "TinyConAttLW":
            self.decoder = TinyLlamaConAttLWDecoder(decoder_config)
            self.lm_head = nn.Linear(decoder_config.hidden_size, config.vocab_size, bias=False)
        elif decoder_config.model_method == "TinyConAttT2B":
            self.decoder = TinyLlamaConAttT2BDecoder(decoder_config)
            self.lm_head = nn.Linear(decoder_config.hidden_size, config.vocab_size, bias=False)
        elif decoder_config.model_method == "TinyConAttT2BPrefix":
            self.decoder = TinyLlamaConAttT2BPrefixDecoder(decoder_config)
            self.lm_head = nn.Linear(decoder_config.hidden_size, config.vocab_size, bias=False)
        elif decoder_config.model_method == "TinyCrossAttLW":
            self.decoder = TinyLlamaCrossAttLWDecoder(decoder_config)
            self.lm_head = nn.Linear(decoder_config.hidden_size, config.vocab_size, bias=False)
        elif decoder_config.model_method == "ConAttT2BPrefix":
            self.decoder = LlamaConAttT2BPrefixDecoder(decoder_config)
            self.lm_head = nn.Linear(decoder_config.hidden_size, config.vocab_size, bias=False)
        else:
            self.decoder = LlamaConAttLWDecoder(decoder_config)
...
class LlamaConcatAttentionEncDec(LlamaForEncDec):
    def __init__(self, config):
        super().__init__(config)

    def set_share_paremeters(self):
        
        def share_layers(share_encoder_layers_id, decoder_layers_id):
            ## share embedding
            self.decoder.embed_tokens = self.encoder.embed_tokens

            decoder_layers = [self.decoder.layers[i] for i in decoder_layers_id]
            encoder_layers = [self.encoder.layers[i] for i in share_encoder_layers_id]
            
            for i, layers in enumerate(zip(encoder_layers, decoder_layers)):
                encoder_layer, decoder_layer = layers
                decoder_layer.input_layernorm = encoder_layer.input_layernorm
                decoder_layer.self_attn.q_proj = encoder_layer.self_attn.q_proj
                decoder_layer.self_attn.k_proj = encoder_layer.self_attn.k_proj
                decoder_layer.self_attn.v_proj = encoder_layer.self_attn.v_proj
                decoder_layer.self_attn.o_proj = encoder_layer.self_attn.o_proj
                decoder_layer.self_attn.rotary_emb = encoder_layer.self_attn.rotary_emb
                decoder_layer.post_attention_layernorm = encoder_layer.post_attention_layernorm
                decoder_layer.mlp = encoder_layer.mlp    
                if torch.cuda.current_device() == 0:
                    print(f"share parameters: Encoder layer {share_encoder_layers_id[i]} ==> Decoder layer {decoder_layers_id[i]}")   

        decoder_param_method = self.config.decoder["decoder_param_method"]
        if decoder_param_method == "share":
            self.decoder.norm = self.encoder.norm
            
            share_encoder_layers_id = self.config.decoder.get("share_encoder_layers_id", None)
            ## old code
            if share_encoder_layers_id is None:
                k = self.config.decoder["share_bottom_k"]
                m = self.config.decoder["share_top_m"]
                middle = self.config.decoder.get("share_middle", 0)
                encoder_layer_num = self.config.num_hidden_layers

                ## share encoder's bottom k layer +  top m layer 
                if middle == 0:
                    sample_middle_layers = []
                ## share  bottom k layer + extract middle layer + top m layer 
                else:
                    interval = (encoder_layer_num - k - m) // middle
                    sample_middle_layers = [k+i*interval for i in range(middle)]
                
                share_encoder_layers_id = list(range(k)) + sample_middle_layers + list(range(encoder_layer_num-m, encoder_layer_num))
            
            decoder_layers_id = list(range(len(self.decoder.layers)))
            share_layers(share_encoder_layers_id, decoder_layers_id)

        elif decoder_param_method == "init":
            pass
            ...
            share_encoder_layers_id = self.config.decoder.get("share_encoder_layers_id", None)
            if share_encoder_layers_id is None:
                print("Random mode require set share_encoder_layers_id!!")
                exit()
            
            decoder_layers_id = list(range(len(self.decoder.layers)))
            share_layers(share_encoder_layers_id, decoder_layers_id)

        elif decoder_param_method == "freeze":
            ## freeze encoder
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        # always keep src + tgt length
        cat_attention_mask = torch.cat([attention_mask, decoder_attention_mask], dim=1)
        ...
        if decoder_attention_mask is not None and position_ids is None:
            # create position_ids on the fly for batch generation
            ## resume position
            position_ids = cat_attention_mask.long().cumsum(-1) - 1

            ## reset position
            # position_ids = torch.cat([attention_mask.long().cumsum(-1)-1, decoder_attention_mask.long().cumsum(-1)-1], dim=1)

            position_ids.masked_fill_(cat_attention_mask == 0, 1)
            if past_key_values:
                position_ids = position_ids[:, -input_ids.shape[1] :]
    ...
    def set_share_paremeters(self):
        ...
        def share_layers(share_encoder_layers_id, decoder_layers_id):
            ...
            for i, layers in enumerate(zip(encoder_layers, decoder_layers)):
                ...
                # decoder_layer.cross_atten_layernorm = encoder_layer.input_layernorm
                # decoder_layer.cross_attn.q_proj = encoder_layer.self_attn.q_proj
                # decoder_layer.cross_attn.k_proj = encoder_layer.self_attn.k_proj
                # decoder_layer.cross_attn.v_proj = encoder_layer.self_attn.v_proj
                # decoder_layer.cross_attn.o_proj = encoder_layer.self_attn.o_proj
                decoder_layer.cross_attn.rotary_emb = encoder_layer.self_attn.rotary_emb
        ...
        if decoder_param_method == "share":
            ...
            share_encoder_layers_id = getattr(self.config.decoder, "share_encoder_layers_id", None)
            ## old code
            if share_encoder_layers_id is None:
                k = self.config.decoder["share_bottom_k"]
                m = self.config.decoder["share_top_m"]
            ...
            for name, param in self.named_parameters():
                ...
                #     param.requires_grad = False
                #     is_freeze = True
                # ## train decoder's cross attention and encoder_project only
                # elif "cross_attn" not in name and "encoder_project" not in name:
                #     param.requires_grad = False
                #     is_freeze = True
            ...
            # ## freeze decoder
            # bottom = 12
            # freeze_decoder_prefix = [f"layers.{i}." for i in range(bottom)] 
            # for name, param in self.decoder.named_parameters():
            #     for prefix in freeze_decoder_prefix:
            #         if prefix in name:
            #             param.requires_grad = False
            #             print(f"freeze Decoder ==> {name}")

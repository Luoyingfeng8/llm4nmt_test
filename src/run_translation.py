...
def main():
    ...
    # PrefixLM
    elif model_args.model_method == "ConAttT2BPrefix":
        if model_args.run_mode == "init":
            # set encoder config
            config.is_encoder_decoder = True
            config.decoder_start_token_id = config.bos_token_id
            config.encoder_method = model_args.encoder_method
            config.model_method = model_args.model_method
            state_dict = utils.make_prefixlm_state_dict(model_path=model_args.model_name_or_path)
    ...
    # layer-wise coordinate
        ...
        if model_args.run_mode == "init":
            ...
            model = LlamaConcatAttentionEncDec.from_pretrained(model_args.model_name_or_path, config=config)
    elif model_args.model_method in ["TinyConAttT2B", "TinyConAttT2BPrefix"]:
        if model_args.run_mode == "init":
            # seting decoder config
            decoder_config = copy.deepcopy(config.to_dict())
            decoder_config["num_hidden_layers"] = model_args.decoder_layer_num
        ...
        ## train the whole decoder
            ...
            model = LlamaCrossAttentionEncDec.from_pretrained(model_args.model_name_or_path, config=config)
            # model.set_share_paremeters() # 冻结实验
    else:
    ...
    if data_args.pad_to_max_length:
        ...
        elif model_args.model_method in ["ConAttLW", "ConAttT2B", "CrossAttLW", "TinyConAttLW", "TinyConAttT2B","TinyConAttT2BPrefix","TinyCrossAttLW","ConAttT2BPrefix"]:

...
@dataclass
class DataCollatorForS2SConAtt:
    tokenizer: PreTrainedTokenizerBase
...
@dataclass
class DataCollatorForS2SBottomConcat:
    tokenizer: PreTrainedTokenizerBase
    model: Optional[Any] = None
    ...
    def __call__(self, features, return_tensors=None):
        ...
        ## some tokenizer may not add eos to the end
        if labels[0][-1] != self.tokenizer.eos_token_id:
            labels = [label + [self.tokenizer.eos_token_id] for label in labels]
        if input_ids[0][-1] != self.tokenizer.eos_token_id:
            input_ids = [ids + [self.tokenizer.eos_token_id] for ids in input_ids]

        ## step1: get decoder_input_ids according labels
        decoder_input_ids = [[self.tokenizer.bos_token_id] + label[:-1] for label in labels]

        ## step2: padding decoder_input_ids with right side and get decoder_attention_mask
        max_length = max(len(l) for l in decoder_input_ids)
        max_length = (
                    (max_length + self.pad_to_multiple_of - 1)
                    // self.pad_to_multiple_of
        ...
        decoder_input_ids = [ids + [self.tokenizer.pad_token_id]*(max_length-len(ids)) for ids in decoder_input_ids]
        decoder_attention_mask = [[0 if x == self.tokenizer.pad_token_id else 1 for x in y] for y in decoder_input_ids]

        ## step3: padding input_ids with left side and get attention_mask
        max_length = max(len(l) for l in input_ids)
        max_length = (
                    (max_length + self.pad_to_multiple_of - 1)
                    // self.pad_to_multiple_of
        ...
        ## step4: construct labels
        src_part = [[self.label_pad_token_id]*len(input_ids[0])] * len(input_ids)
        padding_labels = [ids + [self.tokenizer.pad_token_id]*(len(decoder_input_ids[0])-len(ids)) for ids in labels]
        tgt_part = [[self.label_pad_token_id if x == self.tokenizer.pad_token_id else x for x in y] for y in padding_labels]
        assert len(src_part) == len(tgt_part)
        labels = [x+y for x,y in zip(src_part, tgt_part)]

...
class LlamaConAttT2BPrefixDecoder(LlamaConAttT2BDecoder):
    ...
    def _update_causal_mask(self, attention_mask, input_tensor, cache_position, past_seen_tokens, encoder_attention_mask=None, prefix=False):
        if self.config._attn_implementation == "flash_attention_2":
            if attention_mask is not None and 0.0 in attention_mask:
                return attention_mask
            return None
...
class LlamaForEncDec(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        ## decoder
        ### for share decoder
        decoder_config = PretrainedConfig.from_dict(config.decoder)
